...
def obst(n, keys, freq):
    ...
    for row in cost:
        print(row)
    print("The optimal cost of the binary search tree is :", cost[0][n-1])

n = int(input("Enter no. of nodes: "))
keys = list(map(int, input("Enter the keys: ").strip().split()))[:n]
freq = list(map(int, input("Enter the freq: ").strip().split()))[:n]
obst(n, keys, freq)
